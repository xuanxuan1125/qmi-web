...
// RouteContract is the source-of-truth inventory for the small net/http
// router. Keeping this beside the handler makes API wiring reviewable even
// without a third-party router that can enumerate routes at runtime.
...
// RouteContracts returns a copy so callers cannot change the inventory.
func RouteContracts() []RouteContract {
	return append([]RouteContract(nil), routeContracts...)
}

// RouteReport is checked into docs/ROUTES.md and verified by the contract test,
// preventing the human-readable API inventory from drifting from code.
func RouteReport() string {
	var report strings.Builder
	report.WriteString("# QMI Web v0.3.0 backend route inventory\n")
	...
	for _, route := range RouteContracts() {
		fmt.Fprintf(&report, "%s | %s | %s | %s | %s\n", route.Method, route.Path, route.Handler, route.Access, route.SideEffect)
	}
