# QMI Web API Contract v0.1.1

This contract describes the same-origin API used by the Vue WebUI. It is derived from `internal/api/RouteContracts` and the checked-in route inventory at [`reports/routes-v0.1.1.txt`](../reports/routes-v0.1.1.txt).
...
| --- | --- | --- |
| Overview | `GET /api/v1/dashboard` | `device_status`, `sim`, `signal`, `sms`, `notifications`, `data_guard` |
...
| Diagnostics | `GET /api/v1/diagnostics` | platform, DB, guard, backend, detected/active device metadata |
